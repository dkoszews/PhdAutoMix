...
def cfg():
    ...
    model_config = {"enst_path": "/mnt/windaten/Datasets/ENST_Drums",  # SET ENST PATH HERE
                    ...
                    # sampled from with each batch equals cache_size / num_snippets_per_track. Set as high as your
                    # RAM allows.
                    'num_workers': 4,
                    # Number of processes used for each TF map operation used when loading the dataset
                    "num_snippets_per_track": 100,
                    # Number of snippets that should be extracted from each song at a time after loading it. Higher
                    # values make data loading faster, but can reduce the batches song diversity
                    ...
                    # -1,1] at test time, and might be more stable than tanh
                    'context': False,
                    # Type of padding for convolutions in model. If False, feature maps double or half in dimensions
                    # after each convolution, and convolutions are padded with zeros ("same" padding). If True,
                    # convolution is only performed on the available mixture input, thus the output is smaller than
                    # the input
                    'network': 'unet',  # Type of network architecture
                    'upsampling': 'linear',
                    # Type of technique used for upsampling the feature maps in a unet architecture, either 'linear'
                    # interpolation or 'learned' filling in of extra samples
                    'task': 'dry',
                    # Type of separation task. 'voice' : Separate music into voice and accompaniment.
